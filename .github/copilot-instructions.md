...
- root/ is a special folder for run-time copy and MUST ONLY be used for runtime copy-to-rootfs. It MUST NOT be used by agents.
- app/, etc/ and defaults/ MUST be included at the same level as the docker-bake.hcl. you SHOULD using where possible as a temporary datastore for runtime copy.
