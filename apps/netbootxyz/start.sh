# ===== From ./processed/netbootxyz/root/etc/s6-overlay//s6-rc.d/init-netboot-config/run =====
...
# ===== From ./processed/netbootxyz/root/etc/s6-overlay//s6-rc.d/init-nginx-config/run =====
#!/usr/bin/with-contenv bash
# shellcheck shell=bash
...
# ===== From ./processed/netbootxyz/root/etc/s6-overlay//s6-rc.d/svc-nginx/run =====
#!/usr/bin/with-contenv bash
# shellcheck shell=bash
...
# ===== From ./processed/netbootxyz/root/etc/s6-overlay//s6-rc.d/svc-tftp/run =====
#!/usr/bin/with-contenv bash
# shellcheck shell=bash
...
# ===== From ./processed/netbootxyz/root/etc/s6-overlay//s6-rc.d/svc-webapp/run =====
#!/usr/bin/with-contenv bash
# shellcheck shell=bash

exec \
    s6-notifyoncheck -d -n 300 -w 1000 -c "nc -z localhost 3000" \
        cd /app s6-setuidgid abc /usr/bin/node app.js
