...
if [[ "$DB_TYPE" == "postgres" ]]; then
    ...
    if [[ ! -f "$CONFIG_FILE" ]]; then
        echo "Missing config file, running Prowlarr to ensure configfile can be accessed"

        /app/bin/Prowlarr --nobrowser --data=/config "$@" &
        PROWLARR_PID=$!
        ...
        echo "Config file present, stopping Prowlarr..."
        kill "$PROWLARR_PID"
    fi

    : "${DB_USER:?Need to set DB_USER}"
    : "${DB_PASSWORD:?Need to set DB_PASSWORD}"
    : "${DB_DATABASE:=prowlarr-main}"
    : "${DB_LOGSDB:=prowlarr-log}"
    : "${DB_HOST:=postgres}"
    : "${DB_PORT:=5432}"

    echo "Updating Prowlarr config.xml for PostgreSQL..."
