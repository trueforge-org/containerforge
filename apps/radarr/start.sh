...
if [[ "$DB_TYPE" == "postgres" ]]; then
    ...
    if [[ ! -f "$CONFIG_FILE" ]]; then
        echo "Missing config file, running Radarr to ensure configfile can be accessed"

        /app/bin/Radarr --nobrowser --data=/config "$@" &
        RADARR_PID=$!
        ...
        echo "Config file present, stopping Radarr..."
        kill "$RADARR_PID"
    fi

    : "${DB_USER:?Need to set DB_USER}"
    : "${DB_PASSWORD:?Need to set DB_PASSWORD}"
    : "${DB_DATABASE:=radarr-main}"
    : "${DB_LOGSDB:=radarr-log}"
    : "${DB_HOST:=postgres}"
    : "${DB_PORT:=5432}"

    echo "Updating Radarr config.xml for PostgreSQL..."
