...
if [[ ! -f "/usermod.done" ]] && [[ -n "${USER_NAME}" ]] && [[ "${USER_NAME}" != "apps" ]] && grep -q "^${USER_NAME}" /etc/passwd; then
...
if [[ "$USER_NAME" != "apps" ]]; then
    usermod -l "$USER_NAME" apps
    groupmod -n "$USER_NAME" apps
    touch /usermod.done
fi
...
# set password for apps to unlock it and set sudo access
